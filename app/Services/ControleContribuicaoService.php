...
class ControleContribuicaoService
{
    ...
    public function novoPorPeriodo($dadosForm, $comprovante = null)
    {
        ...
        try {
            ...
            if ($de == $ate && $deAno == $ateAno) {
                $this->pagamento($deAno, $de,
                    $dadosForm['vlcontribuicaoperiodo'],
                    $dadosForm['dtdepositoperiodo'], $comprovante,
                    $dadosForm['tipoContribuicao']);
            } elseif ($de != $ate && $deAno == $ateAno) {
                for ($i = $de; $i <= $ate; $i++) {
                    $this->pagamento($deAno, $i,
                        $dadosForm['vlcontribuicaoperiodo'],
                        $dadosForm['dtdepositoperiodo'], $comprovante,
                        $dadosForm['tipoContribuicao']);
                }
    ...
    function novoPorMes($dadosForm, $comprovante = null)
    {
        ...
        try {
            ...
            $de = $dadosForm['demes'];
            $this->pagamento($deAno, $de, $dadosForm['vlcontribuicaomes'],
                $dadosForm['dtdepositomes'], $comprovante,
                $dadosForm['tipoContribuicao']);
            DB::commit();
            return '{"operacao":true}';
        } catch
    ...
    function pagamento(
        ...
        try {
            ...
            $dados['vl_contribuicao_mes']
                = $this->trataMoeda($valorContribuicaoPeriodo);
            $dados['nu_ano'] = $ano;
            $dados['nu_mes'] = $mes;
            $dados['tp_contribuicao'] = $tp_contribuicao;
            $dados['dt_contribuicao'] = date('Y-m-d',
                strtotime($dtdepositoperiodo));
            ...
            if ($comprovantes != null) {
                foreach ($comprovantes as $comprovante) {
                    $controle->comprovante()
                        ->attach($comprovante, ['st_ativo' => 'S']);
                }
            }
    ...
    public function editar($dadosForm)
    {
        ...
        try {
            ...
            $dados['dt_contribuicao'] = date('Y-m-d',
                strtotime($dadosForm['dtdepositomes']));
            $dados['vl_contribuicao_mes']
                = $this->trataMoeda($dadosForm['vlcontribuicaomes']);
            $this->controleContribuicaoRepository->update($dados,
                $dadosForm['co_seq_controle_contribuicao'],
    ...
    public function desativar($co_seq_controle_contribuicao)
    {
        ...
        try {
            ...
            $this->controleContribuicaoRepository->update($dados,
                $co_seq_controle_contribuicao, 'co_seq_controle_contribuicao');
    ...
    public function reprovaContribuicao(
        $co_seq_controle_contribuicao,
        $motivo = null
    ) {
        DB::beginTransaction();
        ...
        try {
            ...
            if (!empty($motivo)) {
                $dados['ds_observacao_financeiro'] = $motivo;
            ...
            return [
                "operacao" => true,
                'id'       => $co_seq_controle_contribuicao
            ];
        } catch
        (\Illuminate\Database\QueryException $e) {
